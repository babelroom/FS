 ...
 *
 * JR Modified for BabelRoom June/2012
 *
 */
#include <switch.h>
...
SWITCH_MODULE_LOAD_FUNCTION(mod_br_conference_load);
SWITCH_MODULE_SHUTDOWN_FUNCTION(mod_br_conference_shutdown);
SWITCH_MODULE_DEFINITION(mod_br_conference, mod_br_conference_load, mod_br_conference_shutdown, NULL);
...
/* JR BabelRoom */
#define CONF_MAX_GROUPS (25)    /* consider stack size if this is increased */
static inline int br_to_sane_group(int g)
{
    if (g<0)
        return 0;
    if (g>=CONF_MAX_GROUPS)
        return (CONF_MAX_GROUPS-1);
    return g;
}
...
static switch_status_t conference_add_event_member_data(conference_member_t *member, switch_event_t *event)
{
    ...
    /* JR BabelRoom */
	switch_event_add_header(event, SWITCH_STACK_BOTTOM, "Group", "%d", member->group_num);
	switch_event_add_header(event, SWITCH_STACK_BOTTOM, "PA", "%s", switch_test_flag(member, MFLAG_PA) ? "true" : "false" );
...
static void *SWITCH_THREAD_FUNC conference_thread_run(switch_thread_t *thread, void *obj)
{
	...
	while (globals.running && !switch_test_flag(conference, CFLAG_DESTRUCT)) {
        ...
        int max_group_num = 0;      /* JR BabelRoom */
		...
		for (imember = conference->members; imember; imember = imember->next) {
			uint32_t buf_read = 0;
            int tmp = imember->group_num;
			total++;
			imember->read = 0;

            if (tmp>max_group_num && tmp<CONF_MAX_GROUPS) {
                max_group_num = tmp;
                }
		...
		if (ready || has_file_data) {
            ...
            int super_frame[(CONF_MAX_GROUPS) * (SWITCH_RECOMMENDED_BUFFER_SIZE / 2)];  /* JR BabelRoom
			int main_frame[SWITCH_RECOMMENDED_BUFFER_SIZE / 2] = { 0 }; */
            int g;
#define MAIN_FRAME(g)   (&(super_frame[g*(SWITCH_RECOMMENDED_BUFFER_SIZE/2)]))
            int *main_frame;
			int16_t write_frame[SWITCH_RECOMMENDED_BUFFER_SIZE / 2] = { 0 };

            max_group_num = br_to_sane_group(max_group_num);
            memset(super_frame, 0, (max_group_num+1)*(SWITCH_RECOMMENDED_BUFFER_SIZE / 2)*sizeof(int));

			/* Init the main frame with file data if there is any. */
			bptr = (int16_t *) file_frame;
			if (has_file_data && file_sample_len) {
                for(g = 0; g <= max_group_num; g++) {       /* efficient? */
                    main_frame = MAIN_FRAME(g);
				    for (x = 0; x < bytes / 2; x++) {
			...
			for (omember = conference->members; omember; omember = omember->next) {
                ...
                if (switch_test_flag(omember, MFLAG_PA)) {
                    /* copy to all frames */
                    for(g = 0; g <= max_group_num; g++) {
                        main_frame = MAIN_FRAME(g);
                    ...
                    /* copy to the appropriate frame only */
                    main_frame = MAIN_FRAME(br_to_sane_group(omember->group_num));
				    for (x = 0; x < omember->read / 2; x++) {
					    main_frame[x] += (int32_t) bptr[x];
				        }
                    }
			...
			if (conference->agc_level && conference->member_loop_count) {
				...
				conference->score = conf_energy / (      ((bytes*(max_group_num+1)) / 2)     / divisor) / conference->member_loop_count;
...
static void conference_list(conference_obj_t *conference, switch_stream_handle_t *stream, char *delim)
{
	...
	for (member = conference->members; member; member = member->next) {
		...
		stream->write_function(stream, "%u%s%s%s%s%s%s%s%s%s%d%s",
                                member->id, delim, name, delim, uuid, delim, profile->caller_id_name, delim,
                                profile->caller_id_number, delim, member->group_num, delim);
        ...
        /* JR BabelRoom */
		if (switch_test_flag(member, MFLAG_PA)) {
			stream->write_function(stream, "%s%s", count ? "|" : "", "pa");
			count++;
		}
...
static void conference_xlist(conference_obj_t *conference, switch_xml_t x_conference, int off)
{
	...
	for (member = conference->members; member; member = member->next) {
        ...
        /* JR BabelRoom */
		switch_snprintf(i, sizeof(i), "%d", member->group_num);
		add_x_tag(x_member, "group", i, toff++);
		...
		x_tag = switch_xml_add_child_d(x_flags, "pa", count++); /* JR BabelRoom */
		switch_xml_set_txt_d(x_tag, switch_test_flag(member, MFLAG_PA) ? "true" : "false");
...
static switch_status_t conf_api_sub_record(conference_obj_t *conference, switch_stream_handle_t *stream, int argc, char **argv)
{
    ...
    if (argc <= 2 ) {
		switch_channel_t *channel;
	    conference_member_t *imember = conference->members;
	    switch_mutex_lock(conference->member_mutex);
	    if (    conference                          &&
                (imember=conference->members)       &&
                imember->session                    &&
                (channel = switch_core_session_get_channel(imember->session)) ) {
			char *rfile = switch_channel_expand_variables(channel, conference->auto_record);
	        stream->write_function(stream, "Record file (auto format) %s\n", rfile);
	        conference->record_count++;
			launch_conference_record_thread(conference, rfile);
			if (rfile != conference->auto_record) {
				switch_safe_free(rfile);
...
/* --- Added for BabelRoom --- */
static void add_group_info(conference_member_t *member, char **buf1, int *buf1len, char **buf2, int *buf2len)
{
    char buf[128];
    int tmp;
	switch_assert(member != NULL);
    sprintf(buf,"%u,",member->id);
    tmp = strlen(buf);
    *buf1 = realloc(*buf1, (*buf1len)+tmp+1);
    strcpy((*buf1)+(*buf1len),buf);
    *buf1len += tmp;
    sprintf(buf,"%d,",member->group_num);
    tmp = strlen(buf);
    *buf2 = realloc(*buf2, (*buf2len)+tmp+1);
    strcpy((*buf2)+(*buf2len),buf);
    *buf2len += tmp;
}
static switch_status_t conf_api_sub_group(conference_obj_t *conference, switch_stream_handle_t *stream, int argc, char **argv)
{
	switch_status_t ret_status = SWITCH_STATUS_SUCCESS;
    int group_num = -1;
	switch_assert(conference != NULL);
	...
	if (argc>3 && group_num>=0 && group_num<CONF_MAX_GROUPS) {
        conference_member_t *member = NULL;
        switch_event_t *event = NULL;
        char *members = NULL;
        int members_len = 0;
        char *old_groups = NULL;
        int old_groups_len = 0;
	    switch_mutex_lock(conference->member_mutex);
        if (!strcasecmp(argv[3],"all")) {
	        for (member = conference->members; member; member = member->next) {
		        if (member->session && !switch_test_flag(member, MFLAG_NOCHANNEL)) {
                    //switch_thread_rwlock_rdlock(member->rwlock);    /* non-exclusive lock should do us */ -- overkill ???? TODO
                    add_group_info(member, &members, &members_len, &old_groups, &old_groups_len);
		    ...
		    for (x = 3; x < argc; x++) {
                ...
                add_group_info(member, &members, &members_len, &old_groups, &old_groups_len);
                member->group_num = group_num;
                switch_thread_rwlock_unlock(member->rwlock);    /* reader lock acquired by conference_member_get() */
                }
            }
        switch_mutex_unlock(conference->member_mutex);

        /* tell them what happened */
        if (    members && members_len && old_groups && old_groups_len &&
                switch_event_create_subclass(&event, SWITCH_EVENT_CUSTOM, CONF_EVENT_MAINT)==SWITCH_STATUS_SUCCESS) {
            members[members_len-1] = '\0';      /* chomp last comma (,) from end of list */
            old_groups[old_groups_len-1] = '\0';
            conference_add_event_data(conference, event);
            switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "Member-List", members);
            switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "New-Group", argv[2]);
            switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "Old-Group-List", old_groups);
            switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "Action", "group");
            switch_event_fire(&event);
		    stream->write_function(stream, "+OK group: to_group(%d) moved(%s) from(%s)\n", group_num, members, old_groups);
            }
        else {
		    stream->write_function(stream, "+OK group: no members found to move");
            }
        switch_safe_free(members);
        switch_safe_free(old_groups);
	} else {
		stream->write_function(stream, "-ERR group: bad arguments. Group (%d) should be >=%d and <%d\n",
            group_num, 0, CONF_MAX_GROUPS);
		ret_status = SWITCH_STATUS_GENERR;
	}
...
static switch_status_t conf_api_sub_pa(conference_member_t *member, switch_stream_handle_t *stream, void *data)
{
	switch_event_t *event;
	...
	switch_set_flag_locked(member, MFLAG_PA);
	if (stream != NULL) {
		stream->write_function(stream, "+OK pa %u\n", member->id);
	}
	if (switch_event_create_subclass(&event, SWITCH_EVENT_CUSTOM, CONF_EVENT_MAINT) == SWITCH_STATUS_SUCCESS) {
		conference_add_event_member_data(member, event);
		switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "Action", "pa-member");
		switch_event_fire(&event);
	}
...
static switch_status_t conf_api_sub_unpa(conference_member_t *member, switch_stream_handle_t *stream, void *data)
{
	switch_event_t *event;
	...
	switch_clear_flag_locked(member, MFLAG_PA);
	if (stream != NULL) {
		stream->write_function(stream, "+OK unpa %u\n", member->id);
	}
	if (switch_event_create_subclass(&event, SWITCH_EVENT_CUSTOM, CONF_EVENT_MAINT) == SWITCH_STATUS_SUCCESS) {
		conference_add_event_member_data(member, event);
		switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "Action", "unpa-member");
		switch_event_fire(&event);
	}
...
static switch_status_t conf_api_sub_info(conference_member_t *member, switch_stream_handle_t *stream, void *data)
{
	switch_event_t *event;
	...
	if (stream != NULL) {
		stream->write_function(stream, "+OK info %u\n", member->id);
	}
	if (switch_event_create_subclass(&event, SWITCH_EVENT_CUSTOM, CONF_EVENT_MAINT) == SWITCH_STATUS_SUCCESS) {
		conference_add_event_member_data(member, event);
        switch_event_add_header(event, SWITCH_STACK_BOTTOM, "Volume-In", "%d", member->volume_in_level);
        switch_event_add_header(event, SWITCH_STACK_BOTTOM, "Volume-Out", "%d", member->volume_out_level);
        switch_event_add_header(event, SWITCH_STACK_BOTTOM, "AGC-Volume-In", "%d", member->agc_volume_in_level);
		switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "Action", "info-member");
		switch_event_fire(&event);
	}
...
static api_command_t conf_api_sub_commands[] = {
    ...
    /* added for BabelRoom */
	{"group", (void_fn_t) & conf_api_sub_group, CONF_API_SUB_ARGS_SPLIT, "group", "<group#> <all|member id [...<member id>]>"},
	{"pa", (void_fn_t) & conf_api_sub_pa, CONF_API_SUB_MEMBER_TARGET, "pa", "<[member_id|all]|last>"},
	{"unpa", (void_fn_t) & conf_api_sub_unpa, CONF_API_SUB_MEMBER_TARGET, "unpa", "<[member_id|all]|last>"},
	{"info", (void_fn_t) & conf_api_sub_info, CONF_API_SUB_MEMBER_TARGET, "info", "<[member_id|all]|last>"},
};
